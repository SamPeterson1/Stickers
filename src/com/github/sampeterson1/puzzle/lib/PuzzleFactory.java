...
public class PuzzleFactory {
	...
	public static Puzzle createPuzzle(PuzzleType type, int size) {
		return createPuzzle(type, size, false);
	}
	
	public static Puzzle createPuzzle(PuzzleType type, int size, boolean withMetaFunctions) {
		Puzzle puzzle = initializers.get(type).apply(size);
		
		if(withMetaFunctions) 
			puzzle.addMetaFunctions();
